...
public abstract class AbstractStrikeStratagem implements IStratagem {
    ...
    protected void spawnOrbitalVisual(ServerLevel level, Vec3 targetPos, ParticleOptions coreParticle, ParticleOptions trailParticle) {
        ...
        for (int i = 0; i < beamHeight; i++) {
            ...
            level.sendParticles(trailParticle, x, particleY, z, 2, 0.2, 0.2, 0.2, 0.01);
        }
    ...
    protected void strikeRandomInRadius(Level level, Vec3 center, float scatterRadius, float explosionPower, boolean spawnFire, Level.ExplosionInteraction explodeType) {
        strikeRandomInRadius(level, center, scatterRadius, explosionPower, spawnFire, explodeType, true);
    }

    protected void strikeRandomInRadius(Level level, Vec3 center, float scatterRadius, float explosionPower, boolean spawnFire, Level.ExplosionInteraction explodeType, boolean addOrbitalEffect) {
        double offsetX = (level.random.nextDouble() - 0.5) * 2 * scatterRadius;
        double offsetZ = (level.random.nextDouble() - 0.5) * 2 * scatterRadius;
        surfaceStrike(level, center.x + offsetX, center.z + offsetZ, explosionPower,spawnFire,explodeType,addOrbitalEffect);
    }

    protected void surfaceStrike(Level level, double x, double z, float explosionPower, boolean spawnFire, Level.ExplosionInteraction explodeType, boolean addOrbitalEffect){
        int strikeHeight = level.getHeight(Heightmap.Types.MOTION_BLOCKING, (int) x, (int) z);
        strike(level,x,strikeHeight,z,explosionPower,spawnFire,explodeType,addOrbitalEffect);
    }

    protected void strike(Level level, double x, double y, double z, float explosionPower, boolean spawnFire, Level.ExplosionInteraction explodeType, boolean addOrbitalEffect){
        ...
        level.explode(null, x,  y + 0.5, z,
                explosionPower, spawnFire, explodeType);
    }
}